...
public class T_SubrogasiSummaryController {
    ...
    public ResponseEntity<ResponseData<T_Subrogasi_Summary>> create(@RequestBody RequestSubrogasi request){
        ...
        try {
            ...
            if(acs != null){
                ...
                T_Subrogasi_Summary subrogasiSummary2 = subrogasiSummary;
                T_Subrogasi subrogasi= new T_Subrogasi();

                 if(subrogasiSummary2.getLineNo()!=null){
                    System.out.println("Subrogation2 line no :"+subrogasiSummary2.getLineNo());

                    if(request.getCounterAngsuran() != subrogasiSummary2.getLineNo()&& 
                        request.getCounterAngsuran() < subrogasiSummary2.getLineNo()+1){
                        ...
                        if(acs.getAmtSubrogation()!=0){
                            ...
                            subrogasiSummary2.setLineNo(request.getCounterAngsuran());
                            subrogasiSummary2.setNominalSubrogasiPokok(request.getNilaiRecoveries());
                            subrogasiSummary2.setNominalSubrogasLebih(request.getNilaiRecoveries()-acs.getAmtSubrogation());
                            subrogasiSummary2.setNominalSubrogasiPokok(request.getNilaiRecoveries());
                            subrogasiSummary2.setJenisTransaksi(request.getJenisTransaksi());
                            subrogasiSummary2.setKodeBank(request.getKodeBank());
                            subrogasiSummary2.setKodeCabangAskrindo(request.getKodeCabangAskrindo());
                            subrogasiSummary2.setTglNotaKredit(new Date());
                            subrogasiSummary2.setRemark("RECOV"+request.getJenisTransaksi()+"2"+"_"+acs.getBpUnitCode()+request.getNoRekening()+request.getCounterAngsuran());
                            subrogasiSummary2.setSubrogasiId(subrogasiService.save(subrogasi));
                            subrogasiSummary2.setNominalSubrogasiBunga(Double.valueOf(0));
                            subrogasiSummary2.setNominalSubrogasiDenda(Double.valueOf(0));
                            subrogasiSummary2.setNominalPajak(Double.valueOf(0));
                            subrogasiSummary2.setNominalFeeGross(Double.valueOf(0));
                            subrogasiSummary2.setNominalFeeNet(Double.valueOf(0));
                            subrogasiSummary2.setTanggalJurnal(null);
                            subrogasiSummary2.setNoJurnal(null);
                            subrogasiSummary2.setBiayaRekonsiliasi(Double.valueOf(0));
                            subrogasiSummary2.setCollectingAgent(null);
                            subrogasiSummary2.setNoNotaKredit(null);
                            subrogasiSummary2.setmRekeningGiro(null);
                            subrogasiSummary2.setIsNetting((byte) 0);
                            subrogasiSummary2.setStatus("0");

                            ResponseData<T_Subrogasi_Summary> response = new ResponseData<T_Subrogasi_Summary>();
                            response.setStatus("00");
                            response.setMessage("00");
                            response.getData().add(subrogasiSummaryService.save(subrogasiSummary2));
                    ...
                    throw new Exception("getAmtSubrogation != 0");
                }
            }
    ...
    // @PostMapping("/api/v1/shs/subro")
    // public ResponseEntity<ResponseData<V_CLM_INQUIRY_SUBROGATION_CREDIT>> create(@RequestBody RequestSubrogasi request){
